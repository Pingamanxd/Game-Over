...
let embed = new MessageEmbed().setColor("RANDOM").setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setFooter("Pingman <31 " + member.guild.name).setTimestamp()
if(security === "şüpheli") {
...
let data = await new UserData({ UserID: member.id, Name: namedata, Process: "Sunucudan Ayrılma" }); data.save();
});
